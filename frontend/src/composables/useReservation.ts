...
export function useReservation() {
  const items = ref<ReservationDTO[]>([]);
  const item = ref<ReservationDTO | null>(null);
  ...
  const create = async (data: ReservationDTO) => {
    const res = await reservationService.create(data);
    return res.data;
  };

  const update = async (id: number | string, data: ReservationDTO) => {
    const res = await reservationService.update(id, data);
    return res.data;
  };
