import { ref } from 'vue';
import { orderItemService, OrderItemDTO } from '../services/orderItemService';

export function useOrderItem() {
  const items = ref<OrderItemDTO[]>([]);
  const item = ref<OrderItemDTO | null>(null);

  const getAll = async () => {
    const res = await orderItemService.getAll();
    items.value = res.data;
    return res.data;
  };

  const getById = async (id: number | string) => {
    const res = await orderItemService.getById(id);
    item.value = res.data;
    return res.data;
  };

  const create = async (data: OrderItemDTO) => {
    const res = await orderItemService.create(data);
    return res.data;
  };

  const update = async (id: number | string, data: OrderItemDTO) => {
    const res = await orderItemService.update(id, data);
    return res.data;
  };
