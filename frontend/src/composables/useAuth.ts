import { ref } from 'vue';
import { authService, AuthDTO } from '../services/authService';

export function useAuth() {
  const items = ref<AuthDTO[]>([]);
  const item = ref<AuthDTO | null>(null);
  ...
  const create = async (data: AuthDTO) => {
    const res = await authService.create(data);
    return res.data;
  };

  const update = async (id: number | string, data: AuthDTO) => {
