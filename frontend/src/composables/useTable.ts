import { ref } from 'vue';
import { tableService, TableDTO } from '../services/tableService';

export function useTable() {
  const items = ref<TableDTO[]>([]);
  const item = ref<TableDTO | null>(null);
  ...
  const getById = async (id: number | string) => {
    const res = await tableService.getById(id);
    item.value = res.data;
    return res.data;
  };

  const create = async (data: TableDTO) => {
    const res = await tableService.create(data);
    return res.data;
  };

  const update = async (id: number | string, data: TableDTO) => {
